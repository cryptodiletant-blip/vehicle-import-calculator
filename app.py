...
def detect_image_type(base64_string):
    """Detect image type from base64 string with multiple methods"""
    try:
        ...
        # WebP
        if img_data[0:4] == b'RIFF' and img_data[8:12] == b'WEBP':
            return 'image/webp'
        
        # AVIF (check for ftyp box with avif/avis brands)
        if b'ftyp' in img_data[4:12]:
    ...
    # Fallback: try to infer from base64 prefix
